...
def _validated_demo_capabilities(payload: Any) -> list[str]:
    ...
    for receipt in receipts:
        if not isinstance(receipt, dict) or str(receipt.get("status")) != "validated":
            continue
        out.extend(DEMO_CAPABILITY_ALIASES.get(str(receipt.get("demo_id") or ""), []))
...
def main() -> int:
    ...
    cycle.update({
        ...
        "next_action": "revalidate_best_historical_cash_first_candidate" if counts["recovery_candidates"] else "execute_opportunity_factory_query_pack",
    })
