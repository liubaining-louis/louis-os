...
records verifiable evidence, and updates the dashboard ledger. It never claims
revenue or submits work automatically.
"""
...
def score(item: dict[str, Any]) -> float:
    ...
    score_value = 20.0
    if amount > 0:
        score_value += min(40.0, amount / 25.0)
...
def main() -> None:
    ...
    for query in QUERIES:
        ...
        except Exception as exc:  # evidence must include failures too
    ...
    (RESULTS / "monetization_candidates.json").write_text(
        json.dumps({"generated_at": now, "count": len(candidates), "candidates": candidates, "errors": errors}, indent=2),
        encoding="utf-8",
    )
    ...
    with (RESULTS / "monetization_experiments.jsonl").open("a", encoding="utf-8") as handle:
        handle.write(json.dumps(experiment, ensure_ascii=False) + "\n")
    ...
    print(json.dumps(experiment, ensure_ascii=False))
