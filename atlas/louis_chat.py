...
def _save(session_id: str, role: str, text: str, channel: str) -> None:
    ...
    root.collection("messages").add(
        {"role": role, "text": text, "channel": channel, "created_at": now}
    )
...
def _update_summary(session_id: str) -> None:
    ...
    prompt = (
        "Résume cette conversation Louis OS en français en moins de 180 mots. "
        "Conserve objectifs, décisions, contraintes, actions confirmées et points en attente. "
        "N'invente rien.\n\n" + transcript
    )
    try:
...
<input id=key class=key type=password placeholder='Clé Louis OS'><div class=tabs><button onclick=showChat()>Chat</button><button onclick=showMemory()>Mémoire</button><button onclick=newSession()>Nouvelle session</button></div>
...
<script>const log=document.getElementById('log'),key=document.getElementById('key'),msg=document.getElementById('msg'),chat=document.getElementById('chat'),memory=document.getElementById('memory'),memlist=document.getElementById('memlist');key.value=localStorage.louisKey||'';let sid=localStorage.louisSid||crypto.randomUUID();localStorage.louisSid=sid;
function headers(){localStorage.louisKey=key.value;return {'Content-Type':'application/json','Authorization':'Bearer '+key.value}}
...
async function load(){log.innerHTML='';let r=await fetch('/v1/history?session_id='+encodeURIComponent(sid),{headers:headers()});if(r.ok){let j=await r.json();(j.messages||[]).forEach(x=>add(x.text,x.role==='user'?'u':'a'))}}
...
async function loadMemory(){memlist.innerHTML='';let r=await fetch('/v1/memories',{headers:headers()});if(r.ok){let j=await r.json();(j.memories||[]).forEach(x=>{let d=document.createElement('div');d.className='memory';d.textContent='['+x.category+'] '+x.text;memlist.appendChild(d)})}}
...
msg.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send()}});load()</script></body></html>"""


class Handler(BaseHTTPRequestHandler):
    server_version = "LouisChat/2.0"
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if self.path == "/v1/forget":
            ...
            _json(self, 200 if _forget(memory_id) else 404, {"deleted": _forget(memory_id) if False else bool(memory_id)})
            return
        _json(self, 404, {"error": "not_found"})


def main() -> None:
    print(f"Louis Chat 2.0 listening on :{PORT} for project {PROJECT_ID}", flush=True)
